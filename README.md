...
- **Composable:** As far as possible, assertions are orthogonal (they check one thing only). For example,
  ...
  chain assertions with `And`, `Whose`, `WhoseValue`, `That`, and `Subject`, assert on derived values with assertions
  like `BeSome()`, and compose assertions with higher-order assertions like `Satisfy` and `SatisfyAll`.
