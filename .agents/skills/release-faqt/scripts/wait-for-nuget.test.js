...
test('accepts HTTP 200 for the exact normalized version', async () => {
  ...
  const url = await waitForPackage('6.0.0-RC.1', run.options);
  assert.deepEqual({ url, requests: run.requests, sleeps: run.sleeps }, {
    url: 'https://api.nuget.org/v3-flatcontainer/faqt/6.0.0-rc.1/faqt.6.0.0-rc.1.nupkg',
    requests: [{ url: 'https://api.nuget.org/v3-flatcontainer/faqt/6.0.0-rc.1/faqt.6.0.0-rc.1.nupkg', method: 'HEAD' }],
    sleeps: [],
  });
});
...
test('waits through delayed package availability', async () => {
  ...
  await waitForPackage('6.0.0', run.options);
...
test('times out without sleeping or requesting beyond the deadline', async () => {
  ...
  const error = await waitForPackage('6.0.0', run.options).catch(error => error.message);
...
test('fails immediately on unexpected HTTP responses', async () => {
  ...
  assert.deepEqual({ error, calls: run.requests.length, sleeps: run.sleeps }, {
    error: 'Unexpected HTTP 403 while checking Faqt 6.0.0', calls: 1, sleeps: [],
  });
});
